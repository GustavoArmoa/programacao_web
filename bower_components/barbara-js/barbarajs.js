...
barbaraJs.directive('loadingBootstrap', function () {
    return {
        ...
        //</div>
        //
        template : "<div ng-if='loading.show'><div class='progress'> <div class='indeterminate'></div></div></div>"
    };
});
...
barbaraJs.directive('paginationBootstrap', function () {
    return {
        ...
        //</nav>
        //
        template : "<div class='col s12 center' ng-if='pagination.pages > 0'> <ul class='pagination'> <li ng-repeat='page in pagination.pagination' ng-if='page.role' ng-class='page.class'> <a href='' ng-click='pagination.clickAction(page.item)'>{{page.item ? page.item : '...'}}</a> </li></ul> </div>"
    };
});
